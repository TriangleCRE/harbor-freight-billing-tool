...
const PATCHES = [
  ...
  {
    // Leigh Ann confirmed (9/2/26 email) Ataka should be billed starting
    // with the January 2025 cycle -- despite their lease starting April
    // 2024, they weren't actually operating until 2025 and had rent
    // abatement in the meantime. requireExplicitStart is already true on
    // this tenant (set when the meter identity was confirmed, see the
    // 8/31 patch above), which is exactly what keeps them out of every
    // bill until billed_from is set -- this patch sets it, which makes
    // them eligible starting with the first bill whose period end falls
    // in January 2025 onward (matching isEligible()'s own periodEnd-based
    // comparison, same as every other date field in that function).
    id: "2026-09-02-ataka-billed-from-jan-2025",
    async run(client) {
      await client.query(`UPDATE tenants SET billed_from = $2 WHERE id = $1`, [
        "ataka",
        "2025-01-01",
      ]);
    },
