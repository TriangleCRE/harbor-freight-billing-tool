...
// { text: string, types: string[], resolved: boolean, important: boolean }
// entries. Replaces the older flat `notes` / `estimateNote` strings -- a
// bill can now carry several independently-tagged notes instead of one
// blob of text per field.
//
// `resolved` and `important` both back the Flags & Issues tab (public/
// index.html): resolved archives an entry instead of deleting it;
// important is a manual "this one actually needs action" flag Sarah sets
// herself -- there's no reliable way to infer real urgency from note text,
// so this is deliberately a person's judgment call, not a computed guess.
// Both default to false, including for every entry that predates these
// fields -- an absent key normalizes the same as an explicit false, so
// nothing already in the database needs a migration to read as "still open,
// not flagged important".
//
...
function normalizeNoteEntries(b) {
  ...
  const entries = [];
  if (b.notes) entries.push({ text: b.notes, types: ["billingNote"], resolved: false, important: false });
  if (b.estimateNote) entries.push({ text: b.estimateNote, types: ["estimateNote"], resolved: false, important: false });
  return entries;
}
