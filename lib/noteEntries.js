...
// { text: string, types: string[] } entries. Replaces the older flat
// `notes` / `estimateNote` strings -- a bill can now carry several
// independently-tagged notes instead of one blob of text per field.
//
...
function normalizeNoteEntries(b) {
  ...
  const entries = [];
  if (b.notes) entries.push({ text: b.notes, types: ["billingNote"] });
  if (b.estimateNote) entries.push({ text: b.estimateNote, types: ["estimateNote"] });
  return entries;
}
