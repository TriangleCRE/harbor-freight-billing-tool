...
-- Replaces the flat notes / estimate_note strings with a list of
-- independently-tagged entries (see lib/noteEntries.js). ADD COLUMN IF NOT
-- EXISTS is idempotent, same as everything else here, and additive: the
-- old notes / estimate_note columns are left in place (unused by the app
-- from this point on) rather than dropped, so no historical text is lost.
ALTER TABLE bills ADD COLUMN IF NOT EXISTS note_entries JSONB NOT NULL DEFAULT '[]'::jsonb;
`;
